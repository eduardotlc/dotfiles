...
function MdDuFolds(lnum)
  ...
  if string.match(cur_line, "^%#%#.* (%S+.*)") then
    local ntst = string.len(string.match(cur_line, "^%#(%#+)"))
...
-- >> FOLDING TEXT
function MdDuFoldText()
  local line = vim.fn.getline(vim.v.foldstart)
  local folded_line_num = vim.v.foldend - vim.v.foldstart
  local ntst = string.len(string.match(line, "%#+"))
...
vim.opt.foldexpr = "v:lua.MdDuFolds(v:lnum)"
vim.opt.foldtext = "v:lua.MdDuFoldText()"
